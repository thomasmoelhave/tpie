...
bool medium_instance() {
    ...
    get_memory_manager().set_limit(32*1024*1024);
    auto l = tpie::log_info(); progress_indicator_arrow progress("Running test",iterations-1100, l);
...
bool large_instance(){
	...
	progress_indicator_arrow progress("Running test",iterations, l);
...
public:
	...
	virtual void use() {
		auto l = tpie::log_info();
		progress_indicator_arrow progress("Priority queue test",ITEMS*2, l);
...
bool overflow_test() {
	...
	auto l = tpie::log_info();
	progress_indicator_arrow pi("PQ test", 2*steps, l);
	pi.init(2*steps);
