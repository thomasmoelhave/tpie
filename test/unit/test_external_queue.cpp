...
bool empty_size_test() {
	...
	TEST_ENSURE_EQUALITY(0u, q.size(), "Wrong size");
	q.push(0);
	TEST_ENSURE(!q.empty(), "Wrong empty");
	TEST_ENSURE_EQUALITY(1u, q.size(), "Wrong size");
	q.push(0);
	TEST_ENSURE(!q.empty(), "Wrong empty");
	TEST_ENSURE_EQUALITY(2u, q.size(), "Wrong size");
	for (size_t i = 0; i < 1000; ++i) q.push(42);
	TEST_ENSURE(!q.empty(), "Wrong empty");
	TEST_ENSURE_EQUALITY(1002u, q.size(), "Wrong size");
	for (size_t i = 0; i < 1000000; ++i) q.push(42);
	TEST_ENSURE(!q.empty(), "Wrong empty");
	TEST_ENSURE_EQUALITY(1001002u, q.size(), "Wrong size");
