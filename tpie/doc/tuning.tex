%%
%% $Id: tuning.tex,v 1.8 2000-08-08 04:54:53 hutchins Exp $
%%
...
\ref{sec:installation}.\index{configuration:options} None of
...
\begin{description}
    \item[{\myverb{BTE\_IMP\_MMB}}] 
    \item[{\myverb{BTE\_IMP\_STDIO}}]
    \item[{\myverb{BTE\_IMP\_UFS}}] Used to choose which of
    ...
    \myverb{BTE\_IMP\_STDIO}, \myverb{BTE\_IMP\_MMB} or
    \myverb{BTE\_IMP\_UFS}. See Section~\ref{sec:ref-bte}
...
\noindent
If \myverb{BTE\_IMP\_MMB} or \myverb{BTE\_IMP\_UFS} is
...
discussed in the next section):
  ...
  \item[\myverb{BTE\_MMB\_LOGICAL\_BLOCKSIZE\_FACTOR}]
  \item[\myverb{BTE\_UFS\_LOGICAL\_BLOCKSIZE\_FACTOR}] The
  ...
  \item[{\myverb{BTE\_MMB\_READ\_AHEAD}}] 
  \item[{\myverb{BTE\_UFS\_READ\_AHEAD}}] Defining this
  variable instructs the corresponding BTE (\myv{BTE\_mmb}
  or \myv{BTE\_ufs}) to optimize for sequential read speed
...
%  \index{BTE_MMB_READ_AHEAD@{\tt BTE\_MMB\_READ\_AHEAD}}
  ...
  \myverb{BTE\_MMB\_READ\_AHEAD} is defined, defining
  ...
  case of \myverb{BTE\_IMP\_MMB} and not done at all in the
  case of \myverb{BTE\_IMP\_UFS} (refer to
...
\begin{description}
    \item[{\myverb{AMI\_IMP\_SINGLE}}] This macro controls
...
%  \index{AMI_IMP_*@{\tt AMI\_IMP\_*}}
  ...
  \item[{\myverb{AMI\_VIRTUAL\_BASE}}] Defining this macro
  \myverb{AMI\_base\_stream}, which is the base class of all
  implementations of AMI streams, declare its member
  functions to be \myverb{virtual}. This is useful for
  ...
  \item[{\myverb{BTE\_VIRTUAL\_BASE}}] Similar to
  \myverb{AMI\_VIRTUAL\_BASE}, but for the BTE layer.
  ...
  \myverb{AMI\_base\_stream}. Although this base class has
  ...
  set \myverb{AMI\_VIRTUAL\_BASE} to 1 (either in the
  application's configuration, or during compilation by
  using the -D flag). Similarly, \myverb{BTE\_VIRTUAL\_BASE}
...
\myverb{BTE\_mmb} should have the best performance for most
applications, because space and copy time is saved relative
to \myverb{BTE\_stdio} and \myverb{BTE\_ufs} as steam
...
\myverb{BTE\_mmb} whereas it is (typically) done by the OS
in \myverb{BTE\_stdio} and \myverb{BTE\_ufs}.  Also
theoretically, \myverb{BTE\_ufs} (and \myverb{BTE\_mmb})
should perform better than \myverb{BTE\_stdio} because of
...
parameter only applies to \myverb{BTE\_mmb} and
\myverb{BTE\_ufs}).  The size of each buffer and the size of
...
block size to use (in \myverb{BTE\_mmb} and
\myverb{BTE\_ufs}), we have included a C program in the
\myverb{test} directory of the TPIE distribution called
\myverb{bte\_test.c}.  This program can be used to determine
the streaming speeds attained by \myverb{BTE\_stdio},
\myverb{BTE\_mmb}, and \myverb{BTE\_ufs} streams on a given
...
you want to test the streaming speed of \myverb{BTE\_mmb},
\myverb{BTE\_ufs} or \myverb{BTE\_stdio}. Also define the
...
\begin{description}
    ...
    information.
    \item[\myverb{TP\_LOG\_LIB\_INFO}] is the lowest level
...
\myverb{LOG\_DEBUG\_INFO\_ID}({\em msg})
\end{quote}
