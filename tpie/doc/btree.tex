...
%%
%% Btree documentation.
%% $Id: btree.tex,v 1.4 2001-05-11 19:01:26 tavi Exp $
%%

\documentclass[11pt]{article}

\newcommand{\entry}[2]{\> {\tt #1}\\ \>\>\parbox[t]{5.5in}{#2}\\[3mm]}
\newcommand{\btabb}{\begin{tabbing} \hspace*{.3in} \= \hspace{.5in}\=\\ }
\newcommand{\etabb}{\end{tabbing}}
...
\btabb
   ...
   \entry{Btree(const Btree\_params \&params = btree\_params\_default)}
   {Construct an empty Btree using temporary files. The tree is stored in a
   ...
   Btree\_params} class and the default values).}

   \entry{Btree(const char *bfn, BTE\_collection\_type t =
   BTE\_WRITE\_COLLECTION, } { {\tt\qquad\qquad\qquad\qquad const
   Btree\_params \&params = btree\_params\_default)}\\ Construct a Btree
   ...
   \entry{$\sim$Btree()} {Destructor. Either remove or close the supporting
...
\btabb
   ...
   \entry{void load(AMI\_STREAM<Value>* is, float lf = 0.7, float nf = 0.5)}
   ...
   \entry{void load\_sorted(AMI\_STREAM<Value>* is, float lf = 0.7, float nf =
   0.5)} {Same as {\tt load()} above, but bypasses the expensive sorting
   step, by assuming that the stream {\tt is} is sorted.}

   \entry{void sort(AMI\_STREAM<Value>* is, AMI\_STREAM<Value>* \&os)} {As a
   ...
   \entry{void load(Btree<Key, Value, Compare, KeyOfValue>* bt,}
   {{\tt\qquad\qquad\qquad\qquad float leaf\_fill = .7, float node\_fill =
   .5)} \\ Bulk load from another Btree. This is a means of reoganizing a
   Btree after a lot of updates. A newly loaded structure may use less
   space and may answer range queries faster.}

   \entry{void unload(AMI\_STREAM<Value>* s)} {Write all elements stored in
...
%   successfully.}\\[3mm]
   ...
   this Btree object. If {\tt p} is {\tt PERSIST\_DELETE}, all files
   associated with the tree will be removed, and all the elements stored in
   the tree will be lost after the destruction of this Btree object. If
   {\tt p} is {\tt PERSIST\_PERSISTENT}, all files associated with the tree
   will be closed during the destruction of this Btree object, and all the
   information needed to reopen this tree will be saved.}

   \entry{const Btree\_params\& get\_params() const} {Return a const
   reference to the {\tt Btree\_params} object used by the Btree. This
...
\section{Appendix: The {\tt Btree\_params} Class}

The {\tt Btree\_params} class encapsulates all user-definable Btree
...
\btabb
  ...
  \entry{Btree\_params()} {Initialize a {\tt Btree\_params} object with
