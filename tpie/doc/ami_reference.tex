%%
%% $Id: ami_reference.tex,v 1.18 2005-11-23 15:14:06 adanner Exp $
%%

\chapter{TPIE Programmer's Reference}
\plabel{cha:reference}

\comment{LA: Andy and Jan please check if this chapter is ok}

\comment{LA: Add progress bar stuff?! (and what about in section 4?).
  Jan?}
...
types:\comment{LA: Add something like this about persistance flag}
\begin{itemize}
...
\comment{LA: Jan please check (new substream). Also please add
  ...
  \entry{AMI\_err \textbf{new\_substream}(AMI\_stream\_type st, off\_t
    sub\_begin, off\_t sub\_end, AMI\_stream<T> **sub\_stream );} {A
...
\index{seek()@{\tt seek()}!AMI|(} \entry{AMI\_err \textbf{seek}(off\_t
  off);} {Move the current position to \lstinline|off|.}
...
polymorhhs.\comment{LA: Syntax of comparator?}  The comparison
...
\comment{LA: Andy please check + add 2x sort (others?)}
...
prototype:\comment{LA: ``prototype''?}
\begin{lstlisting}
  inline int compare (const KEY & k1, const KEY & k2);
\end{lstlisting}
...
\comment{LA: Andy please check + add STL stuff}
...
quicksort\index{quicksort} algorithm (randomized).\comment{LA:
  Really?}  These routines are used by the external memory sorting
...
\subsection{Description}
External stacks are implemented\comment{LA: Bad word here} through the
templated class \lstinline|AMI_stack<T>|, which is a subclass of
\lstinline|AMI_STREAM<T>|.\comment{LA: Do we care here?} As a
