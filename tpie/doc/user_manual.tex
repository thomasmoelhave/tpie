%%
%% $Id: user_manual.tex,v 1.26 2003-11-27 19:53:15 jan Exp $
%%
\chapter{Overview}
...
In order to cope with the high cost of accessing data, efficient EM
algorithms exploit locality in their design.  They access a large
\emph{block} of $B$ contiguous data elements at a time and perform the
necessary algorithmic steps on the elements in the block while in the
high-speed memory. The speedup can be considerable.  A second
effective strategy for EM algorithms is the use of multiple parallel
disks; whenever an input/output operation is performed, $D$ blocks are
transferred in parallel between memory and each of the $D$ disks (one
block per disk).
...
\texttt{tpie\_\version/lib/libtpie.a}, the test and sample
programs in directory \texttt{tpie\_\version/test}, 
and certain header files in \texttt{tpie\_\version/include}.
...
\lstinputlisting[numbers=left,basicstyle=\ttfamily\small,caption={Code taken from \texttt{tpie\_\version/test/sample\_pgm.cpp}}]{../test/sample_pgm.cpp}
...
transparently by one of three possible \emphd{block transfer
   engines (BTEs)}. Briefly, the file \path"app_config.h" chooses
...
use TPIE but are not familiar with the \CPP{}\index{C++}
...
\lstinputlisting[basicstyle=\ttfamily\small,firstline=1,lastline=28]{src/scan_count.cpp}
...
\lstinputlisting[basicstyle=\ttfamily\small,firstline=30,lastline=48]{src/scan_count.cpp}
...
\lstinputlisting[basicstyle=\ttfamily\small,firstline=1,lastline=26]{src/scan_square.cpp}
...
\lstinputlisting[basicstyle=\ttfamily\small,firstline=28,lastline=55]{src/scan_square.cpp}
...
\lstinputlisting[numberstyle=\tiny,stepnumber=5,basicstyle=\ttfamily\small]{src/scan_sum.cpp}
...
\lstinputlisting[basicstyle=\ttfamily\small,firstline=57,lastline=75]{src/scan_square.cpp}
...
from one.  \index{scanning!ASCII I/O|)}
...
\lstinputlisting[basicstyle=\ttfamily\small]{src/complex.cpp}
\index{scanning!multi-type|)}
...
\lstinputlisting[basicstyle=\ttfamily\small]{src/scan_binary_merge.cpp}
...
\lstinline|operate()|.\comment{LA: The AMI\_merge stuff should
...
\lstinputlisting[basicstyle=\ttfamily\small,firstline=1,lastline=19]{src/my_merger.cpp}
...
\lstinputlisting[basicstyle=\ttfamily\small,firstline=20,lastline=48]{src/my_merger.cpp}
...
\lstinputlisting[basicstyle=\ttfamily\small,firstline=1,lastline=16]{src/mergemgr.cpp}
...
\lstinputlisting[basicstyle=\ttfamily\small,firstline=18,lastline=28]{src/mergemgr.cpp}
...
\lstinputlisting[basicstyle=\ttfamily\small,firstline=30,lastline=39]{src/mergemgr.cpp}
...
\lstinputlisting[basicstyle=\ttfamily\small,firstline=41,lastline=45]{src/mergemgr.cpp}
...
\lstinputlisting[basicstyle=\ttfamily\small,firstline=47,lastline=71]{src/mergemgr.cpp}
...
\lstinputlisting[basicstyle=\ttfamily\small,firstline=73,lastline=96]{src/mergemgr.cpp}
...
\lstinputlisting[basicstyle=\ttfamily\small,firstline=23,lastline=51]{src/complex.cpp}
...
\lstinline|AMI_sort| as follows: \comment{LA: Check that this is
  correct.}

\lstinputlisting[basicstyle=\ttfamily\small]{src/compare_re_class.cpp}
...
% sorting function\\ %
% \lstinline|AMI_optimized_sort()| which has three polymorphs. It is planned that
% after thorough testing this function will be the standard \lstinline|AMI_sort()|
...
% \lstinline|AMI_optimized_sort()| and \lstinline|AMI_sort()|.

% The first two polymorphs of \lstinline|AMI_optimized_sort()| are identical to
% the first two \lstinline|AMI_sort()| polymorphs, which relies on the \lstinline|<|
...
% \lstinline|key| field), and that the class of this field have a well-defined
% (possibly via overloading) \lstinline|<| operator. For example, consider the
% class \lstinline|rectangle| below meant for axis parallel rectangles:
...
\lstinputlisting[basicstyle=\ttfamily\small]{src/rectangle.cpp}
...
\lstinputlisting[basicstyle=\ttfamily\small,firstline=1,lastline=15]{src/sortmanager.cpp}
 
Assuming that the size of each \lstinline|double| is 8 bytes,
we can sort the stream as follows:
 
\lstinputlisting[basicstyle=\ttfamily\small,firstline=17,lastline=24]{src/sortmanager.cpp}
...
\lstinputlisting[basicstyle=\ttfamily\small]{src/reverse_order.cpp}
...
\lstinputlisting[basicstyle=\ttfamily\small,firstline=1,lastline=13]{src/bit_matrix.cpp}
 
Now we simply construct a permutation management object from the
matrices and perform the permutation.

\lstinputlisting[basicstyle=\ttfamily\small,firstline=15,lastline=17]{src/bit_matrix.cpp}
...
\lstinline|AMI_matrix|,\index{AMI_matrix@{\tt AMI\_matrix}}
which is a subclass of
\lstinline|AMI_STREAM|.\index{AMI_STREAM@{\tt AMI\_STREAM}}
...
\lstinputlisting[basicstyle=\ttfamily\small]{src/fill_upper_tri.cpp}
...
\lstinputlisting[basicstyle=\ttfamily\small]{src/matrix_arithmetic.cpp}
...
\lstinputlisting[basicstyle=\ttfamily\small]{src/stream_arithmetic.cpp}
...
\item The following statements are used to indicate that the TPIE
  ...
  amount.\comment{LA: Something about how TPIE counts memory?} This
