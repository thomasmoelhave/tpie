%%
%% $Id: user_manual.tex,v 1.17 1999-11-15 17:59:15 hutchins Exp $
%%
\chapter{Overview}
...
Directory & Contents \\
\hline
 \verb|include|  & The TPIE header files.\index{header files}\\ 

 \verb|lib|      & The TPIE run-time library.  This is relatively small, as most
                   of the TPIE system remains in the form of templated header
                   files.\index{library} \\
 \verb|lib/src|  & The source code for the TPIE run-time
                   library. \\
 \verb|test|     & A series of test applications designed to verify
                   ...
                   Chapter~\ref{ch:examples}.\index{test
                   applications}  \\
 \verb|doc|      & Written documentation for TPIE,
...
\verb|app_config.h| is the TPIE configuration file. TPIE's
\verb|AMI_STREAM| stream I/O operations are carried out
transparently by one of three possible \emph{block transfer
   engines (BTEs)}. Briefly, the \verb|app_config.h| chooses
a specific BTE, and the amount of internal memory used as
buffer space for each \verb|AMI_STREAM|. The
\verb|app_config.h| configuration file is further discussed
in Section~\ref{sec:tuning} which also contains a discussion
of how to choose a BTE for a given platform. The file
\verb|<ami.h>| contains TPIEs templated classed and
functions, while the file \verb|<quicksort.h>| contains
various quicksort polymorphs. Note that each
\verb|AMI_STREAM| corresponds to an underlying Unix file.

The program illustrates the most fundamental
\verb|AMI_STREAM| member functions \verb|read_item()|,
\verb|write_item()|, \verb|seek()| and \verb|persist()|.
Successful execution of these member functions is indicated
by a return value of \verb|AMI_ERROR_NO_ERROR|.  The program
...
(\verb|source.persist(PERSIST_DELETE)|), while the bucket
streams are saved (made persistent with
\verb|buckets[i].persist(PERSIST_PERSISTENT)|) in the
default scratch directory \verb|/var/tmp|. The default
location can be changed by setting the environment variable
\verb|AMI_SINGLE_DEVICE| appropriately (see
...
manager \verb|MM_manager| to prevent the program's internal
