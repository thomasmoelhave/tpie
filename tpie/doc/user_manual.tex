%%
%% $Id: user_manual.tex,v 1.20 2000-06-20 22:23:26 hutchins Exp $
%%
\chapter{Overview}
...
\index{External memory algorithms} Familiarity with the
...
an {\em operation management object},\index{operation management object}
...
\label{sec:omos}\index{operation management object}
...
numbers.  \comment{DH: why is ii public? Can we think of a
   better name for ii(applies to all examples)? Why not
   delete the default value for max for simplicity?}
...
\myverb{AMI\_scan()} can operate on up
...
\myverb{in\_scan->operate()}, just as it would for any scan
object.  Each time \myverb{in\_scan->operate()} is called,
