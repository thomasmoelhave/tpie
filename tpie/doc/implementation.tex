%%
%% $Id: implementation.tex,v 1.18 2002-08-01 01:48:21 tavi Exp $
%%
...
%\myv{AMI\_partition\_and\_merge()}\index{AMI\_partition\_and\_merge@{\tt
...
implementations:
\begin{enumerate}
    \item \myv{BTE\_stream\_stdio}, which uses the
    UNIX \myv{stdio} library as its file access method;
    
    \item \myv{BTE\_stream\_ufs}, which performs I/O via UNIX {\tt
       read()/write()} system calls;
    
    \item \myv{BTE\_stream\_mmap}, which uses the UNIX
    \myv{mmap()/munmap()} calls to perform I/O.
\end{enumerate}

The choice of BTE in a given application is controlled by
compile-time variables in the \myv{app\_config.h} file.
The \myv{BTE\_stream\_stdio} implementation is selected by
defining the variable \myv{BTE\_STREAM\_IMP\_STDIO}, the
\myv{BTE\_stream\_ufs} implementation by defining
\myv{BTE\_STREAM\_IMP\_UFS}, and the memory mapped implementation
by defining \myv{BTE\_STREAM\_IMP\_MMAP}.  For example, the
following code (taken from \myv{app\_config.h} selects
the \myv{BTE\_stream\_ufs} Block Transfer Engine (and does not
select the others):
...
of \myv{AMI\_stream\_base} to specify what type of streams they
are. 
...
\myv{BTE\_STREAM\_IMP\_UFS}\comment{DH: We should change the default
...
\myv{BTE\_stream\_base} (in file
\myv{bte\_stream\_base.h}) and must support the member
...
\myv{BTE\_stream\_ufs}, \myv{BTE\_stream\_stdio} or \myv{BTE\_stream\_mmap} are
...
The \myv{BTE\_stream\_ufs}, \myv{BTE\_stream\_stdio}, and
\myv{BTE\_stream\_mmap} implementations are defined in the
respective files \myv{include/bte\_stream\_ufs.h},
\myv{include/bte\_stream\_stdio.h}, and \myv{include/bte\_stream\_mmap.h}.
...
The common BTE member functions\footnote{%
   These functions can be defined as \myv{virtual} to
   assist in debugging a new BTE if the compile-time
   variable \myv{BTE\_VIRTUAL\_BASE} is defined to be
   non-zero (see also file \myv{app\_config.h}). For best
   run-time performance, the default value for
   \myv{BTE\_VIRTUAL\_BASE} is zero.  } 
...
for the stream constructor to be \myv{virtual}. This is a
problem in C++, and so the ``pseudo-constructor'' approach
described here is used instead.\comment{LA: Extend like in
   AMI ref.  Stream type?}
Because \myv{new\_substream()} is not a constructor, but rather a
...
stream in \myv{elt}. Since the application data elements are blocked,
...
\end{verbatim}
Writes the data element pointed to by \myv{elt} to the
...
\emphd{file pointer}) can be modified via the \myv{seek}
...
stream. The BTE \myv{seek} member function is similar to
...
\myv{truncate} function of the underlying file access
method. Note: \myv{truncate} is not supported for substreams.
...
\myv{usage\_type} purposes. On return, \myv{usage}
points to the value used. The valid values for \myv{usage\_type}
are as follows:\comment{LA: ? DH: DONE}
\begin{itemize}
\item \myv{MM\_STREAM\_USAGE\_OVERHEAD}: the size in
bytes of the stream object plus, if this is not a substream,
the stream header block.
\item \myv{MM\_STREAM\_USAGE\_BUFFER}: the number of
bytes consumed by block buffers (blocks of elements in main memory).
\item \myv{MM\_STREAM\_USAGE\_CURRENT}:
\myv{MM\_STREAM\_USAGE\_OVERHEAD} plus the number of
bytes currently consumed by block buffers for this stream. 
\item \myv{MM\_STREAM\_USAGE\_MAXIMUM}:
\myv{MM\_STREAM\_USAGE\_OVERHEAD} plus the maximum number of
bytes that will be consumed by block buffers for this
stream.
\item \myv{MM\_STREAM\_USAGE\_SUBSTREAM}:
The same as \myv{MM\_STREAM\_USAGE\_OVERHEAD} but assumes
...
\begin{itemize}
\item \myv{BTE\_STREAM\_STATUS\_NO\_STATUS}: No status
information exists for the stream.  
\item \myv{BTE\_STREAM\_STATUS\_INVALID}: An error was
encountered while manipulating the stream and the stream
can no longer be used.
%UNUSED \item \myv{BTE\_STREAM\_STATUS\_EOS\_ON\_NEXT\_CALL}:
\item \myv{BTE\_STREAM\_STATUS\_END\_OF\_STREAM}: The end
of stream was encountered.
\end{itemize}
The \myv{get\_status} member function is one of the few
that is implemented in \myv{bte\_stream\_base.h} and its
implementation is therefore common to all BTEs. The status
is stored in the private variable \myv{status} of a BTE object. 
...
value of \myv{persistence}. This may be one of the
following values:
\begin{itemize}
\item \myv{PERSIST\_DELETE}: Delete the stream from the
disk when the stream 
  destructor is called.
\item \myv{PERSIST\_PERSISTENT}: Do not delete the stream from the disk when
  the stream destructor is called.
%\item \myv{PERSIST\_READ\_ONCE}: Delete each block of data from the disk as
%  it is read.
\end{itemize}
...
\myv{/include/bte\_stdio.h}.  \myv{BTE\_stream\_stdio}
streams are stored as ordinary UNIX files which are
manipulated via the \emphd{stdio} file access method, i.e.
via the standard \myv{C} I/O library. The read/write
primitives of \myv{BTE\_stream\_stdio} streams are
implemented using the system calls \myv{fread} and
\myv{fwrite}. The underlying operating system blocking
...
The \myv{BTE\_stream\_stdio} class inherits from
\myv{BTE\_stream\_base} (in file
\myv{/include/bte\_stream\_base.h}):\index{BTE_stream_base@{\tt
      BTE\_stream\_base}}
\begin{verbatim}
...
\myv{BTE\_stream\_stdio} defines the public member
functions defined for \myv{BTE\_stream\_base} (please
...
\myv{BTE\_stream\_stdio} defines its own
...
\myv{BTE\_stream\_mmap}, defined in file
\myv{/include/bte\_stream\_stdio.h}.  \myv{BTE\_stream\_mmap}
streams are stored as ordinary UNIX files.  The TPIE
\myv{BTE\_stream\_mmap} implementation uses the Unix
\myv{mmap} and \myv{munmap} calls to perform I/O. The
UNIX \myv{mmap} call allows a part of a file to be
...
\myv{BTE\_stream\_mmap} primitives explicitly maintain the
...
The \myv{BTE\_stream\_mmap} is not limited to mapping in
...
used by \myv{BTE\_stream\_mmap} can be set using the macro
\myv{BTE\_STREAM\_MMAP\_BLOCK\_FACTOR} in the
\myv{app\_config.h} file. However, choosing a large
...
Unlike in \myv{BTE\_stream\_stdio}, where a function call
is incurred every time a stream object is accessed, the
\myv{BTE\_stream\_mmap} only incurs such a call on every
...
\myv{BTE\_stream\_stdio}, \myv{BTE\_stream\_mmap} has
to implement its own prefetching scheme. (Note on the other
hand that unlike in \myv{BTE\_stream\_stdio} objects does
not pass through the kernel level buffer space on its way to
user space).  \myv{BTE\_stream\_mmap} prefetching can be
turned on by defining the macro
\myv{BTE\_MMAP\_READ\_AHEAD} in the \myv{app\_config.h}
file. If this compile-time variable is defined,
\myv{BTE\_stream\_mmap} optimize for sequential read speed
...
\begin{itemize}
    ...
    \item If the \myv{USE\_LIBAIO} macro is undefined
    (and \myv{BTE\_MMAP\_READ\_AHEAD} is set), read ahead
    is done using \myv{mmap} calls.

    \item If the \myv{USE\_LIBAIO} macro is defined (and
    \myv{BTE\_MMAP\_READ\_AHEAD} is set), read ahead is
...
\end{itemize}
By default \myv{BTE\_MMAP\_READ\_AHEAD} is defined,
\myv{USE\_LIBAIO} is not.\comment{LA: Discuss the two and
   how they differ.}

\myv{BTE\_stream\_mmap} class inherits from \myv{BTE\_stream\_base}:\index{BTE_stream_base@{\tt BTE\_stream\_base}}
\begin{verbatim}
...
\myv{BTE\_stream\_mmap} defines the public member
functions defined for \myv{BTE\_stream\_base} (please see
...
\index{BTE_stream_mmap@{\tt BTE\_stream\_mmap}!constructors}
...
The \myv{BTE\_stream\_mmap} header\index{BTE_stream_mmap@{\tt
      BTE\_stream\_mmap}!header}\index{streams!header block}
...
\myv{/include/bte\_stdio.h}.  \myv{BTE\_stream\_ufs}
streams are stored as ordinary UNIX files.
\myv{BTE\_stream\_ufs} streams use
\myv{read()}/\myv{write()} calls to implement their
I/O.


% The motivation behind implementing \myv{BTE\_ufs} streams
% is an empirically observed inefficiency\footnote{The
%    inefficiency can occur on account of various reasons. On
%    one system, \myv{mmap()} calls were implemented on top
%    of the \myv{stdio} interface instead of a direct I/O
%    implementation, resulting in extra overhead.} of
% \myv{mmap()} implementations in some systems.\comment{LA:
%    Say that its Solaris?} In such situations, another stream
% implementation that, like \myv{BTE\_stream\_mmap} streams,
% have the potential of exploiting large sized blocks and
% buffers is needed.

% The \myv{BTE\_ufs} stream implementation simulates the
% \myv{BTE\_stream\_mmap} stream implementation: Whenever the latter
% maps in a new block, the former reads in a new block (via
% \myv{read()}) and whenever the latter unmaps a block, the
% latter attains the same result via a \myv{write()} call.
% But the \myv{BTE\_ufs} implementation involves explicitly
% keeping track in the BTE code various things which are
% ``under the hood'' in \myv{mmap()}
% implementations.\comment{LA: Like what?} In fact, the code
% in the \myv{BTE\_ufs} implementation can be said to
% amount to a (very rudimentary) \myv{mmap()}
% implementation. 

Like in the case of \myv{BTE\_stream\_mmap} the
(logical) block size can be controlled using the macro
\myv{BTE\_STREAM\_UFS\_BLOCK\_FACTOR}.
\myv{BTE\_stream\_ufs} also has the same advantage as
\myv{BTE\_stream\_mmap} over \myv{BTE\_stream\_stdio} of only incurring
one kernel call per block. However, unlike \myv{BTE\_stream\_mmap}
(but like \myv{BTE\_stream\_stdio}), prefetching is done
...
In \myv{BTE\_stream\_ufs} streams, when the asynchronous I/O library
\myv{libaio}\index{libaio library@{\tt libaio} library.} is available,
there is a provision to do (user-level) prefetching within \myv{BTE\_stream\_ufs}
streams but we do not recommend its use on account of the implicit
filesystem prefetching.

\myv{BTE\_stream\_ufs} class inherits from
\myv{BTE\_stream\_base}:
\begin{verbatim}
...
\myv{BTE\_stream\_ufs} defines the public member
functions defined for \myv{BTE\_stream\_base} (please see
Section~\ref{sec:imp-bte-base} for a list of these member
functions and their semantics).  In addition to these it
defines its own constructors: 
...
\begin{verbatim}
  ...
  BTE_stream_ufs(BTE_stream_ufs *super_stream,
                 ...
                 off_t sub_begin, off_t sub_end);
\end{verbatim}
The \myv{BTE\_stream\_ufs} header structure is as follows:
...
memory as requested by the \myv{new} and \myv{delete} operators,
...
\myv{MM\_manager}, is built from the source files
\myv{mm\_register.cpp}, \myv{mm\_base.cpp},
\myv{mm\_register.h}, and \myv{mm\_base.h}.  The memory
...
\comment{(Need some comments on the current simple MM that we have and some OS issues
...
an \myv{mmap} stream are desired, the code would be similar
to the following:
\begin{verbatim}
...
\myv{AMI\_STREAM} keyword.  \myv{AMI\_STREAM} is a macro
...
\myv{AMI\_STREAM} is stored in a standard UNIX file on a
...
a single disk). The string \myv{AMI\_STREAM} is
\myv{\#define}'d to be the string
\myv{AMI\_stream\_single} in the header file
\myv{ami\_imps.h}. This arrangement is intended to permit
alternative implementations of \myv{AMI\_STREAM} if
...
\begin{itemize}
    ...
    \begin{enumerate}     
        ...
        \myv{AMI\_stream\_base} as \myv{virtual} if compile
...
% The public member functions of the \myv{AMI\_STREAM}
...
% \myv{AMI\_STREAM} is a macro that resolves to a class
...
% data elements stored in \myv{AMI\_STREAM}s.  An
% \myv{AMI\_STREAM} can be viewed as simply a wrapper
% around a \myv{BTE\_STREAM} (See
% Section~\ref{BTEinterface} for a discussion of the BTE
% interface).

%as a \myv{BTE\_STREAM}, where BTE stands for \emphd{block transfer
%engine.} Each \myv{BTE\_STREAM} is a stream built on top
%of a Unix file, with  
...
%different \myv{BTE\_STREAM} implementations: The implementations vary
...
%we focus on the \myv{AMI\_STREAM} interface since that
%is most directly 
%relevant to a programmer/user.

%The member functions of the \myv{AMI\_STREAM} class,
%templated on the type T of its items,  are defined in the file \myv{ami\_single.h};
%some important related enumerated types that the user will be
% concerned with have been defined in \myv{ami\_base.h}.

%An important consequence of the construction of an \myv{AMI\_STREAM} is a
...
%dedicated to that \myv{AMI\_STREAM} and its related data
...
%by an \myv{AMI\_STREAM} often plays an important role while making such 
%a choice. The memory manager \myv{mm.h} is responsible for keeping
%track of the amount of memory avaoable to the user at any time.

%When an \myv{AMI\_STREAM} is destroyed using a destructor, that
%amount of memory becomes is added to the available memory for the
%user. 


%Sometimes the
%items in an \myv{AMI\_STREAM} will never be used  once
%the \myv{AMI\_STREAM} is 
%destructed whereas sometimes an ``idle''
%\myv{AMI\_STREAM} may be destroyed  
...
%destroy or delete the Unix file backing the \myv{AMI\_STREAM} to save
%disk space whereas in the latter case we need the Unix file backing
%the \myv{AMI\_STREAM} to exist on disk even when the
%   \myv{AMI\_STREAM} is 
%destroyed. The  \myv{AMI\_STREAM} destructor looks at the persistence 
...
The file \myv{include/ami\_scan.h} defines the scanning
functionality provided by TPIE. This file is mechanically generated
by \myv{make} when TPIE is compiled and built. It is essentially a concatenation
of the files \myv{include/ami\_scan.h.head},
\myv{include/ami\_scan\_mac.h}, and
\myv{include/ami\_scan.h.tail} -- for more
details, see the file \myv{include/Makefile}. It is not
intended that \myv{include/ami\_scan.h} be modified
manually.
...
\myv{ami\_merge\_base.h}

%\subsubsection{Merge Management Objects}
%
%Merge management objects based on the super class \myv{AMI\_merge\_base} 
%defined in the file \myv{ami\_merge.h} can be used conveniently by
...
\myv{ami\_merge.h}\comment{LA: Remember something about ami\_merge really
...
The \myv{AMI\_merge()} polymorphs (defined in file
\myv{ami\_optimized\_merge.h}) work without a merge
...
in file \myv{include/mergeheap.h}) which exploits the
fact that \myv{delete-min} and \myv{insert} are always
performed together. This improves efficiency. In
 \myv{AMI\_key\_merge()} the heap stores the
...
\myv{AMI\_partition\_and\_merge()} repeatedly merges together the maximum
possible number of sub-streams using \myv{AMI\_merge()}. An important point
is that the substreams input to \myv{AMI\_merge()} during the execution of
\myv{AMI\_partition\_and\_merge()} all originate from the same underlying
parent \myv{AMI\_STREAM}; thus filesystem accesses are inherently
non-sequential. Throughout the execution of
\myv{AMI\_partition\_and\_merge()}, there are only two active
\myv{AMI\_STREAM}s at any time: One which stores the substreams being
...
\end{verbatim}
    ...
    \myv{AMI\_sort\_V1()} are implemented using
    \myv{AMI\_partition\_and\_merge()} and a merge
    management object. The \myv{merge\_sort\_manager}
    class, a base class for merge management objects needed in
    \myv{AMI\_sort\_V1()}, is also defined in file
    \myv{include/ami\_sort\_single.h}.  The member functions
    \myv{main\_mem\_operate()}, \myv{initialize()},
    and \myv{operate()} of the merge management object
    provide the sorting-specific details required for merge
    sorting:
    \begin{itemize}
        \item \myv{main\_mem\_operate()} is simply an
        in-memory sorting algorithm based on quicksort (in
        file \myv{quicksort.h}).
        
        \item Member function \myv{operate()} selects the
        next output element during the merging of runs using
        a standard priority queue (in file
        \myv{pqueue\_heap.h}).
        
        \item Member function \myv{initialize()} just
...
All \myv{sort\_manager} objects contain the following
member functions:
\begin{itemize}
    ...
    in the case of \myv{AMI\_ptr\_sort} and an array of keys
    in the case of \myv{AMI\_key\_sort}).
    
    \item member function \myv{main\_mem\_operate}, which
    ...
    which cleans up after \myv{main\_mem\_operate} (e.g.
    frees the data structures used after an internal memory
    sort).
    
    \item member function \myv{single\_merge}, which merges
    a set of input streams into an output stream. This
    resolves to a call to \myv{AMI\_single\_merge\_dh} in
...
All \myv{sort\_manager} objects also contain a
...
One important difference between the \myv{AMI\_sort\_V1()}
and the \myv{AMI\_sort()}, \myv{AMI\_ptr\_sort()}, and
\myv{AMI\_key\_sort()} implementations is in the way they
...
the \myv{AMI\_sort\_V1()} implementations which only have
...
for merging by the \myv{AMI\_sort()} function all have
...
% the \myv{AMI\_optimized\_sort()} polymorph which relies
...
% As the \myv{AMI\_sort()} polymorphs are based on
% \myv{AMI\_partition\_and\_merge()} which in turn are
% based on \myv{AMI\_merge()} and a merge management
% object, the \myv{AMI\_optimized\_sort()} polymorphs are
% based on the \myv{AMI\_merge()} polymorphs that do not
% use a standard (as described in
% Section~\ref{sec:ref-ami-merge}) TPIE merge management
% object. Thus \myv{AMI\_optimized\_sort()} benefits from
% the efficiency improvement obtained in
% \myv{AMI\_optimized\_merge()} relative to
% \myv{AMI\_merge()}. \myv{AMI\_optimized\_sort()} is a
% merge sort implemented along the lines of
% \myv{AMI\_partition\_and\_merge()} (in fact the
% implementation of \myv{AMI\_optimized\_sort()} really is
% an \myv{AMI\_partition\_and\_merge()} polymorphs defined
% in the file \myv{ami\_optimized\_merge.h}).
