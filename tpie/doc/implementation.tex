%%
%% $Id: implementation.tex,v 1.15 2001-04-26 23:11:37 hutchins Exp $
%%
...
implementations:
\begin{enumerate}
    \item \myv{BTE\_stdio}, which uses the
    UNIX \myv{stdio} library as its file access method.
    
    \item \myv{BTE\_ufs}, which performs I/O via UNIX {\tt
       read/write} calls,
    
    \item \myv{BTE\_mmb}, which uses the UNIX
...
the \myv{BTE\_ufs} Block Transfer Engine (and does not
select the others):
...
\myv{BTE\_base\_stream} (in file
\myv{bte\_base\_stream.h}) and must support the member
...
\myv{BTE\_ufs}, \myv{BTE\_stdio} or \myv{BTE\_mmb} are
...
The \myv{BTE\_ufs}, \myv{BTE\_stdio}, and
\myv{BTE\_mmb} implementations are defined in the
respective files \myv{include/bte\_ufs.h},
\myv{include/bte\_stdio}, and \myv{include/bte\_mmb.h}.
...
that is implemented in \myv{bte\_base\_stream.h} and its
...
\myv{BTE\_base\_stream} (in file
\myv{/include/bte\_base\_stream.h}):\index{BTE_base_stream@{\tt
      BTE\_base\_stream}}
\begin{verbatim}
...
functions defined for \myv{BTE\_base\_stream} (please
...
\myv{/include/bte\_stdio.h}.  \myv{BTE\_stream\_mmb}
...
\myv{BTE\_stream\_mmb} class inherits from \myv{BTE\_base\_stream}:\index{BTE_base_stream@{\tt BTE\_base\_stream}}
\begin{verbatim}
...
functions defined for \myv{BTE\_base\_stream} (please see
...
\myv{BTE\_stream\_mmb} over \myv{BTE\_stdio} of only incurring
one kernel call per block. However, unlike \myv{BTE\_stream\_mmb}
(but like \myv{BTE\_stdio}), prefetching is done
...
In \myv{BTE\_ufs} streams, when the asynchronous I/O library
\myv{libaio}\index{libaio library@{\tt libaio} library.} is available,
there is a provision to do (user-level) prefetching within \myv{BTE\_ufs}
...
\myv{BTE\_base\_stream}:\comment{LA: Why called
...
\begin{verbatim}
  ...
  BTE_single_disk(BTE_single_disk *super_stream,
                 ...
                 off_t sub_begin, off_t sub_end);
\end{verbatim}
The \myv{BTE\_ufs} header structure is as follows:
...
functionality provided by TPIE. This file is a concatenation
of the files \myv{include/ami_scan.h.head},
\myv{include/ami_scan_mac.h}, and
\myv{include/ami_scan.h.tail}, and is mechanically generated
by \myv{make} when TPIE is compiled and built (for more
