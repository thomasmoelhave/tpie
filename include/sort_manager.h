...
#ifndef _SORT_MANAGER_H
#define _SORT_MANAGER_H
...
class sort_manager{
  ...
  private:
    ...
    AMI_err single_merge(AMI_STREAM<T>**, arity_t, AMI_STREAM<T>*, TPIE_OS_OFFSET = -1);
    ...
    I InternalSorter;             //Method for sorting runs in memory
    M MergeHeap;                  //Merge heap implementation 
...
AMI_err sort_manager<T,I,M>::start_sort(){
  ...
  if (nInputItems < InternalSorter.MaxItemCount(mmBytesAvail)){
    ...
    InternalSorter.allocate(static_cast<TPIE_OS_SIZE_T>(nInputItems));
    ...
    if ((ae = InternalSorter.sort(inStream, outStream, static_cast<TPIE_OS_SIZE_T>(nInputItems)))
        != AMI_ERROR_NO_ERROR) {
...
AMI_err sort_manager<T,I,M>::merge_to_output(void){
  ...
  //   mrgArity*sizeof(AMI_STREAM<T>*) + space_overhead()[fixed cost]
