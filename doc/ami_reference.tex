%%
%% $Id: ami_reference.tex,v 1.8 2000-08-08 04:55:51 hutchins Exp $
%%
...
class.  A stream of objects of class \myverb{foo} called
\myverb{foo\_stream} is created in a TPIE program as follows:
...
The \myverb{AMI\_STREAM} class has the following public
member functions: \comment{LA: Change when parallel disks
   are introduced}
...
\begin{itemize}
\item{Constructors (and Destructor)}\index{AMI_STREAM@{\tt AMI\_STREAM}!constructors|(}
...
%of type \myverb{AMI\_READ\_WRITE\_STREAM} 
is constructed on a file with a randomly generated name. If
\myverb{path-name} is given and the file does not already
...
\myverb{AMI\_READ\_STREAM}, \myverb{AMI\_WRITE\_STREAM},
\myverb{AMI\_APPEND\_STREAM}, or
\myverb{AMI\_READ\_WRITE\_STREAM}, which define what
operations will be permitted on the stream.\comment{LA:
   ...
   be cleaned up DH: to be confirmed!?} If \myverb{st} is
not specified, the stream type defaults to
\myverb{AMI\_READ\_WRITE\_STREAM}. The
\myverb{AMI\_APPEND\_STREAM} type is like
\myverb{AMI\_WRITE\_STREAM} type\comment{LA: It this true?
   DH: seems ok} except that if the stream is constructed on
...
TPIE application programmer\comment{LA: Why do we have it
   then? DH: for completeness}, constructs a stream from an existing
\myverb{BTE\_STREAM} (see Section~\ref{sec:ref-bte}). The
new \myverb{AMI\_STREAM} gets the same type as the \myverb{BTE\_STREAM}.
\index{AMI_STREAM@{\tt AMI\_STREAM}!constructors|)}

\item{Substreams}\index{substreams!of AMI streams|(}
...
A substream can be created via the pseudo-constructor
\myverb{new\_substream()}, which is declared as follows:
(recall that an AMI\_STREAM normally inherits from AMI\_stream\_single.)\comment{LA: Why is it
called AMI\_stream\_single? Change! DH: reason explained}
\begin{verbatim}
...
{\em The\comment{LA: I don't understand this paragraph} reason we do not use a
real constructor is to get around the fact that constructors can not be
virtual. Because \myverb{new\_substream()} is not a constructor, but rather a
...
\item{Reading and Writing Streams}
...
an array.\comment{LA: Why is there a difference in the
   passing of len? } \index{read_array()@{\tt
      read\_array()}!AMI|)}\index{write_array()@{\tt
      write\_array()}!AMI|)}
...
Apart from by using \myverb{write\_item()} a stream can be extended
(truncated) using the function \myverb{truncate()}.
...
objects. \myverb{current\_item} will be moved to the end of the stream.
...
stream (in bytes). \myverb{usage\_type} is one of
\myverb{MM\_STREAM\_USAGE\_CURRENT}, \myverb{MM\_STREAM\_USAGE\_MAXIMUM}, and
\myverb{MM\_STREAM\_USAGE\_SUBSTREAM} which gives the current use, the maximal
...
\myverb{smo} is a pointer to a scan management object of used defined
...
\myverb{T}.  The polymorph that does this looks as follows
\begin{verbatim}
...
\myverb{AMI\_scan()} to call.\comment{LA: Something about subclass of
AMI\_scan\_object (also in previous text)?} The first is
\index{initialize()@{\tt initialize()}}
\begin{verbatim}
    AMI_err initialize(void);
\end{verbatim}
\myverb{initialize()} initializes the object to prepare it for a scan.  This
member function is called once by each call to \myverb{AMI\_scan()} in order
to initialize the scan management object before any data processing takes
place. This function should return \myverb{AMI\_ERROR\_NO\_ERROR} if
...
passed to the polymorph of \myverb{AMI\_scan()} with which this scan
management object is to be used.

If there are inputs, \myverb{sfin} points to an array of flags, one for each
...
the next call to \myverb{operate()}.  This permits out of step
...
streams. The output flags must be set by \myverb{operate()} to indicate which
outputs are valid and should be written to the output streams. 

The return value of \myverb{operate()} will normally be one
of the following:
\begin{itemize}
    \item \myverb{AMI\_SCAN\_CONTINUE}:
    \index{AMI_SCAN_*@{\tt AMI\_SCAN\_*}} indicates that the
    function should be called again with any ``taken'' inputs
    replaced by the next objects from their respective
    streams
    \item \myverb{AMI\_SCAN\_DONE}: indicates that the scan
...
been processed.  \index{operate()@{\tt operate()}|)}
\comment{LA: Something about special built-in scan
   management objects, e.g, ASCII input/output}
...
TPIE entry point \myverb{AMI\_merge()} allows an arbitrary
...
\comment{LA: Understandable? DH: rewritten.},
...
The parameters of \myverb{AMI\_merge()} are as follows:
\begin{itemize}
...
\index{AMI_partition_and_merge()@{\tt
...
\myverb{operate} member function of a merge management
object (the object of class \myverb{MergeMgr} in the two
merge prototypes listed above). TPIE also provides for an
\myverb{initialize} member function, which it calls at the
beginning of the merge.

A merge management object
must provide \myverb{initialize()} and \myverb{operate()} member
...
\end{verbatim}
The arguments to \myverb{initialize()} are \myverb{arity}, the number of
input streams in the merge, \myverb{in} a pointer to an array of
pointers to input objects, each of which is the first objects appearing
in one of the input streams,  \myverb{taken\_flags} an array of flags
indicating which of the inputs are present (i.e. which of the input
streams is not empty), and a pointer to an output object.

The typical behavior of an \myverb{initialize()} member function for a
merge management object is to place all the input objects into a data
structure and then return \myverb{AMI\_MERGE\_READ\_MULTIPLE} to indicate
that it took all the inputs which were indicated to be valid by
\myverb{taken\_flags}.  \myverb{initialize} need not process all inputs; it
can turn off any flags in \myverb{taken\_flags} corresponding to inputs
that should be presented to \myverb{operate()}.  Alternatively, it can
set \myverb{taken\_index} to the index of a single input it processed and
return \myverb{AMI\_MERGE\_CONTINUE}\index{AMI_MERGE_*@{\tt AMI\_MERGE\_*}}.

The \myverb{operate()} member function for a merge management object is
called repeatedly to process input objects.  Typically, this member
function will choose one input object to process, and set
\myverb{taken\_index} to the index of the pointer to that object in the input
...
\myverb{operate()} should return \myverb{AMI\_MERGE\_OUTPUT}, otherwise, it
returns either \myverb{AMI\_MERGE\_CONTINUE} to indicate that more input should
be presented, or \myverb{AMI\_MERGE\_DONE} to indicate that the merge has
completed.

Alternatively, \myverb{operate()} can clear the elements of
\myverb{taken\_flags} that correspond to inputs it does not currently wish to
process, and then return \myverb{AMI\_MERGE\_READ\_MULTIPLE}.  This is generally
undesirable because, if only one input is taken, it is far slower than
using \myverb{taken\_index} to indicate which input was taken.  The merge
...
\myverb{AMI\_partition\_and\_merge()} must provide three additional member
...
\index{main_mem_operate()@{\tt main\_mem\_operate()}}
\begin{verbatim}
    AMI_err main_mem_operate(T* mm_stream, size_t len);
\end{verbatim}
where \myverb{mm\_stream} is a pointer to an array of objects that have been read
into main memory, and \myverb{len} is the number of objects in the array. The two
...
\index{space_usage_overhead()@{\tt space\_usage\_overhead()}}
\index{space_usage_per_stream()@{\tt space\_usage\_per\_stream()}}
\begin{verbatim}
...
\plabel{sec:ref-miami-merge}\index{merging!without a merge
   management object|)}

\comment{LA: merging without merge management object in index}

\index{AMI_merge()@{\tt AMI\_merge()}|(}
By using appropriate \myverb{operate()} and \myverb{main\_mem\_operate()}
functions, \myverb{AMI\_merge()} can be used to conveniently program
operations other than standard comparison based merging. TPIE also provides
three other \myverb{AMI\_merge()} polymorphs for the special (normal) case of
...
%\myverb{AMI_partition_and_merge()} function except that it requires 
...
%                                           )
%\end{verbatim}
%has the same requirements as the \myverb{MIAMI_single_merge_cmp()} function.
%
...
%                                               KEY dummykey)
%\end{verbatim}
%provides similar functionality as \myverb{AMI_partition_and_merge()}
...
\myverb{AMI\_sort} by \myverb{AMI\_ptr\_sort}. The three
