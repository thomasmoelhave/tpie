%%
%% $Id: implementation.tex,v 1.10 2000-08-08 04:55:19 hutchins Exp $
%%
...
\emph{header block}\index{streams!header block}, contains
...
\myverb{virtual} to assist in debugging a new BTE if the
compile-time variable \myverb{BTE\_VIRTUAL\_BASE} is defined
to be non-zero (see also file \myverb{app\_config.h}). For
...
\emph{file pointer}) can be modified via the \myverb{seek}
...
% the \emph{Access Method Interface (AMI)}, which accesses
...
\emph{merge heap} object, which is custom-chosen by the
...
\hline
\end{tabular}}
\caption{\plabel{fig:components} Customization of Arguments
