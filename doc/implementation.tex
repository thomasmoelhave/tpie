%%
%% $Id: implementation.tex,v 1.5 1999-07-02 16:32:23 large Exp $
%%
...
(MM)\index{memory manager}.  The BTE handles disk block transfers
...
%each running on a single processor
...
cache\index{buffer cache}, which must support concurrent access to files
...
buffer replacement policies.\comment{LA: Does the reader know what this
is?} To further improve performance, some implementations of the BTE move
data from disk directly into user space rather than using a kernel level
buffer cache\index{buffer cache}.  This saves both main memory space and
copying time.\comment{LA: Something about currently on single disk but more
to come?}
...
The AMI\index{access method interface} is a layer between the BTE and user
...
The\comment{LA: This whole section needs an overhaul - discuss
implementation!} BTE is the lowest layer of TPIE. It is the layer that is
...
\subsection{BTE\_mmb}\comment{LA: Why not called BTE\_mmp?}
