%%
%% $Id: implementation.tex,v 1.19 2003-11-27 19:56:08 jan Exp $
%%
...
\lstinline|BTE_STREAM_IMP_UFS|\comment{DH: We should change the default
...
\path"include/bte_stream_mmap.h".  \comment{LA: Some note about
  construction being BTE dependent - why are they? DH: DONE}
...
\emphd{file pointer}) can be modified via the \lstinline|seek|
...
\lstinline|BTE_stream_stdio| defines the public member
...
member functions and their semantics).  \comment{LA: Extend
   - discuss implementation!} Additionally,
...
The \lstinline|BTE_stream_mmap| is not limited to mapping in
...
\begin{itemize}
    ...
    requires the asynchronous I/O library \lstinline|libaio|.\index{libaio library@{\tt libaio} library.}

\end{itemize}
By default \lstinline|BTE_MMAP_READ_AHEAD| is defined,
\lstinline|USE_LIBAIO| is not.\comment{LA: Discuss the two and
   how they differ.}
...
\lstinline|libaio|\index{libaio library@{\tt libaio} library.} is available,
...
\comment{JV: Most of the file has changed. Someone has to update this
  section accordingly.}

\lstinputlisting[caption={The AMI configuration file.}]{../include/ami.h}
%%The relevant portion of \path"include/ami.h" is included below:
...
The file \path"include/ami_scan.h" defines the scanning
...
\path"include/ami_merge.h"\comment{LA: Remember something about ami\_merge really
...
The \lstinline|AMI_key_sort| template is:
\begin{lstlisting}
...
details or options required.
   ...
   Also in this file are the following (templated) class definitions:
    ...
    \begin{itemize}
        ...
        and \lstinline|AMI_ptr_sort| templates.\comment{DH:
...
%        \item \lstinline|sort_manager_kop|: used by the operator
%        variant of the \lstinline|AMI_ptr_sort| template.
%
%        \item \lstinline|sort_manager_kobj|: used by the
%        comparison function variant of the
%        \lstinline|AMI_ptr_sort| template.
...
All \lstinline|sort_manager| objects contain the following
member functions:
\begin{itemize}
...
All \lstinline|sort_manager| objects also contain a
...
\path"include/mergeheap_dh.h". The following table shows the
...
\hline
\lstinline|AMI_sort| & operator & \lstinline|sort_manager_op| & \lstinline|merge_heap_dh_op| \\
\lstinline|AMI_sort| & object   & \lstinline|sort_manager_obj| & \lstinline|merge_heap_dh_obj| \\
\lstinline|AMI_sort| & function & \lstinline|sort_manager_cmp| & \lstinline|merge_heap_dh_cmp| \\
\lstinline|AMI_ptr_sort| & operator & \lstinline|sort_manager_op| & \lstinline|merge_heap_pdh_op| \\
\lstinline|AMI_ptr_sort| & object   & \lstinline|sort_manager_obj| & \lstinline|merge_heap_pdh_obj| \\
\lstinline|AMI_ptr_sort| & function & \lstinline|sort_manager_cmp| &
\lstinline|merge_heap_pdh_cmp| \\
\lstinline|AMI_key_sort| &   & \lstinline|sort_manager_kobj| & \lstinline|merge_heap_dh_kobj| \\
\hline
\end{tabular}}
\caption{\plabel{fig:imp-sort-variants} Customization of Arguments
   to \lstinline|AMI_partition_and_merge_dh|.}
\end{minipage}
\end{center}
\end{figure}


\subsubsection{File: \texttt{include/apm\_dh.h}}
...
\step{1.} {If \lstinline|sort_fits_in_memory|
    \step{(a.)} { \lstinline|main_mem_operate_init| }
    \step{(b.)} { \lstinline|main_mem_operate| }
    \step{(c.)} { \lstinline|main_mem_operate_cleanup| }}
\step{}{else
...
% the \lstinline|AMI_optimized_sort()| polymorph which relies
...
% As the \lstinline|AMI_sort()| polymorphs are based on
% \lstinline|AMI_partition_and_merge()| which in turn are
% based on \lstinline|AMI_merge()| and a merge management
% object, the \lstinline|AMI_optimized_sort()| polymorphs are
% based on the \lstinline|AMI_merge()| polymorphs that do not
% use a standard (as described in
% Section~\ref{sec:ref-ami-merge}) TPIE merge management
% object. Thus \lstinline|AMI_optimized_sort()| benefits from
% the efficiency improvement obtained in
% \lstinline|AMI_optimized_merge()| relative to
% \lstinline|AMI_merge()|. \lstinline|AMI_optimized_sort()| is a
% merge sort implemented along the lines of
% \lstinline|AMI_partition_and_merge()| (in fact the
% implementation of \lstinline|AMI_optimized_sort()| really is
% an \lstinline|AMI_partition_and_merge()| polymorphs defined
% in the file \lstinline|ami_optimized_merge.h").

\subsubsection{File: \texttt{include/mergeheap\_dh.h}}
