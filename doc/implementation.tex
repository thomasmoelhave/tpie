%%
%% $Id: implementation.tex,v 1.20 2005-11-23 15:14:06 adanner Exp $
%%
...
\CPP{},\comment{LA: why?}  and so the ``pseudo-constructor'' approach
described here is used instead.\comment{LA: Extend like in AMI ref.
  Stream type?}  Because \lstinline|new_substream()| is not a
...
as the value of the \emphd{file pointer}) can be modified via the
...
\emphd{stdio} file access method, i.e.  via the standard \lstinline|C|
...
\lstinline|BTE_stream_stdio| defines the public member functions
defined for \lstinline|BTE_stream_base| (please see
Section~\ref{sec:imp-bte-base} for a list of these member functions
and their semantics).  \comment{LA: Extend - discuss implementation!}
...
For\comment{LA: What does this subsection do/say? Seem to be missing
  something. And does it work??} instance, if a single implementation
...
accessed:\comment{DH: There may be a better place for some of the
  stuff in this intro}\comment{LA: Portability somewhere?}
...
AMI\comment{LA: Jan please look at this subsection} stream objects are
...
\tobeextended\comment{LA: Andy please check this section}

\path"include/ami_merge_base.h"\comment{LA: true?}
...
\path"include/ami_merge.h"\comment{LA: Remember something about
  ami\_merge really called ami\_single\_merge.}
...
\tobeextended\comment{ami\_merge\_sorted\_runs?}
...
The\comment{LA: Andy please check this again}\comment{LA: Add 2x
  sorting somewhere. Does progress bar stuff also go here or is it
  more general?} TPIE sort implementation is a multi-way merge sort.
Merge sort consists of two phases: the run formation phase and the
merging phase. During the \emphd{run formation phase}, the $N$ input
elements are read approximately $M$ (one \emphd{memory-load}) at a
...
%When implementing the TPIE merge sort, we had a number of design
%decisions to make. For example, what internal memory sorting algorithm
%should be used, how the merge heap should be implemented, how the
%sorted runs should be stored and so on. In our design, we have tried
%to remain flexible while not sacrificing performance. 


\paragraph{Sort manager.}\comment{LA: I moved this from the end
  (seemed much more natural).} The sort manager class in
...
runs to merge at one time.\footnote{Often the number of streams that
  ...
  at once in a given OS.}\comment{LA: Footnote really true?} The
...
provided until a single output run (stream) is produced.\comment{LA:
  ...
  (AMI\_merge)?} Various combinations of internal sort class and merge
heap are exposed as easier to use \lstinline|AMI_sort| functions
described in \ref{fig:imp-sort-variants}.\comment{LA: This sentence
  (and figure) is not clear - especially now that its moved up from
  bottom. Rewrite, or probably better, have a paragraph at the end
  with this last sentence/stuff and a listing of all the different
  sort calls (like in other sections).}
...
by\comment{LA: ``in''?} an internal sort class. Three different
internal sort class objects, defined in
\path"include/internal_sort.h", are used in TPIE.\comment{LA: Is it
  clear why there is more than one?} Each class object has a public
function
\begin{lstlisting}
...
the quicksort algorithm.\comment{LA: Write more?} Note that the
...
insert.\comment{LA: Extend a little?}

Similar to the internal sort class objects, TPIE implements different
variants\comment{LA: Should it say ``class object'' or something like
  about? If so then also change below} of the mergeheap class that
...
\tobewritten\comment{LA: Add queue}
