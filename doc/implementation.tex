%%
%% $Id: implementation.tex,v 1.17 2002-06-25 18:31:20 tavi Exp $
%%
...
\begin{enumerate}
    ...
    \item \myv{BTE\_stream\_mmb}, which uses the UNIX
    \myv{mmap} call to perform I/O
\end{enumerate}

The choice of BTE in a given application is controlled by
compile-time variables in the \myv{app\_config.h} file.
The UNIX \myv{stdio} implementation is selected by
defining the variable \myv{BTE\_IMP\_STDIO}, the
\myv{read/write} implementation by defining
\myv{BTE\_IMP\_UFS}, and the memory mapped implementation
by defining \myv{BTE\_IMP\_MMB}.  For example, the
...
\myv{BTE\_IMP\_STDIO}\comment{DH: We should change the default
...
\myv{BTE\_stream\_ufs}, \myv{BTE\_stream\_stdio} or \myv{BTE\_stream\_mmb} are
...
\myv{BTE\_stream\_mmb} implementations are defined in the
respective files \myv{include/bte\_stream\_ufs.h},
\myv{include/bte\_stream\_stdio}, and \myv{include/bte\_stream\_mmb.h}.
...
\myv{/include/bte\_stream\_stdio.h}.  \myv{BTE\_stream\_mmb}
streams are stored as ordinary UNIX files.  The TPIE
\myv{BTE\_stream\_mmb} implementation uses the Unix
...
\myv{BTE\_stream\_mmb} primitives explicitly maintain the
...
The \myv{BTE\_stream\_mmb} is not limited to mapping in
...
used by \myv{BTE\_stream\_mmb} can be set using the macro
\myv{BTE\_MMB\_LOGICAL\_BLOCKSIZE\_FACTOR} in the
...
\myv{BTE\_stream\_mmb} only incurs such a call on every
...
\myv{BTE\_stream\_stdio}, \myv{BTE\_stream\_mmb} has
...
user space).  \myv{BTE\_stream\_mmb} prefetching can be
turned on by defining the macro
\myv{BTE\_MMB\_READ\_AHEAD} in the \myv{app\_config.h}
file. If this compile-time variable is defined,
\myv{BTE\_stream\_mmb} optimize for sequential read speed
...
\begin{itemize}
    ...
    (and \myv{BTE\_MMB\_READ\_AHEAD} is set), read ahead
    is done using \myv{mmap} calls.

    \item If the \myv{USE\_LIBAIO} macro is defined (and
    \myv{BTE\_MMB\_READ\_AHEAD} is set), read ahead is
...
\end{itemize}
By default \myv{BTE\_MMB\_READ\_AHEAD} is defined,
\myv{USE\_LIBAIO} is not.\comment{LA: Discuss the two and
   how they differ.}

\myv{BTE\_stream\_mmb} class inherits from \myv{BTE\_stream\_base}:\index{BTE_stream_base@{\tt BTE\_stream\_base}}
\begin{verbatim}
class BTE_stream_mmb : public BTE_stream_base {
  private:
...
\myv{BTE\_stream\_mmb} defines the public member
...
The \myv{BTE\_stream\_mmb} header\index{BTE_stream_mmb@{\tt
...
% implementation that, like \myv{BTE\_stream\_mmb} streams,
% have the potential of exploiting large sized blocks and
% buffers is needed.

% The \myv{BTE\_ufs} stream implementation simulates the
% \myv{BTE\_stream\_mmb} stream implementation: Whenever the latter
...
Like in the case of \myv{BTE\_stream\_mmb} the
(logical) block size can be controlled using the macro
\myv{BTE\_UFS\_LOGICAL\_BLOCKSIZE\_FACTOR}.
\myv{BTE\_stream\_ufs} also has the same advantage as
\myv{BTE\_stream\_mmb} over \myv{BTE\_stream\_stdio} of only incurring
one kernel call per block. However, unlike \myv{BTE\_stream\_mmb}
